...
ADDR_DEVICE = 0x0050   # 32 bytes: SW ver (8), HW ver (8), date (16)
...
LIST_5TONE_INFO_FUNC = ['Mute', 'Stun', 'Kill', 'Wake', 'Emergency']
LIST_5TONE_INFO_RSP = ['None', 'Beep', 'Beep+Reply']
_MDC_SYS_LISTS = ['List 1', 'List 2', 'List 3', 'List 4', 'List 5']
...
def _decode_tone(datH, datL):
    """Decode a raw two-byte tone field to (mode, value, polarity).

    datH bit7 set → DCS: bit6 set = inverted polarity, bits[2:0] hold
    code bits[10:8], datL holds the lower 8 code bits.
    datH bit7 clear → CTCSS stored as BCD-over-hex (0x0885 = 88.5 Hz).
    Returns ('', 0, 'N') when the field is blank (0x00 or 0xFF).
    """
...
def _encode_tone(mode, tone, pol):
    """Encode (mode, tone, polarity) to a two-byte tone field.

    Inverse of _decode_tone: CTCSS encoded as BCD-over-hex,
    DCS encoded with bit7 set and optional polarity flag in bit6.
    """
...
def _encode_freq_le(hz):
    ...
    return struct.pack('<I', min(max(0, hz) // 10, 0xFFFFFFFF))
...
def _bs_rows_to_rgb565(rows):
    """Convert a list of RGB pixel rows to packed RGB565 big-endian bytes."""
    buf = bytearray()
    for row in rows:
        for r, g, b in row:
            # Pack as RGB565 BE: bits 15-11=R(5), 10-5=G(6), 4-0=B(5)
            # High byte first (big-endian pixel order)
            px = ((r >> 3) << 11) | ((g >> 2) << 5) | (b >> 3)
...
def _bs_read_bmp(path):
    """Read a 24-bit BMP file without Pillow.

    Returns (rows, width, height) where rows is a list of row lists of
    (r, g, b) tuples, with bottom-up rows flipped to top-down.
    """
...
def _upload_boot_screen_5rh(radio, img_data):
    ...
    # Drain any trailing bytes after the wake ACK (same firmware quirk as
    # main clone)
    pipe.timeout = 0.05
    pipe.read(16)
...
def _xwrite(pipe, data, seed):
    """XOR each byte with seed and write to pipe."""
    pipe.write(bytes(b ^ seed for b in data))


def _xread(pipe, n, timeout=2.0):
    """Read exactly n bytes from pipe, raising RadioError on timeout."""
    buf = b''
...
def _wait_ack(pipe, seed, label, timeout=2.0):
    """Read one XOR'd byte and verify it decodes to ACK (0x41)."""
    b = _xread(pipe, 1, timeout)
    decoded = b[0] ^ seed   # 0x41 ('A') when XOR'd back is a valid ACK
    if decoded != _ACK:
        LOG.debug('%s: raw=0x%02x seed=0x%02x decoded=0x%02x '
                  '(expected 0x%02x^seed=0x%02x)',
...
def _do_handshake(pipe, seed, direction='R'):
    """Execute the 5-step XOR handshake and return the 16-byte model string.

    Steps: wake pulse → PROGRAM header (seed embedded) → password placeholder
    → INFORMATION magic → direction byte.  Falls back to 115200 baud if the
    radio does not ACK at 19200.
    """
...
def _download_5rh(radio):
    ...
    model_bytes = _do_handshake(pipe, seed, 'R')
    model_str = model_bytes.split(b'\x00')[0].split(b'\xff')[0].decode(
        'ascii', errors='replace')
...
def _mwrite(mmap, addr, data):
    mmap.set(addr, bytes(data))


def _mread(mmap, addr, n):
    return bytearray(mmap[addr:addr + n])
...
class BaofengUV5RMPlusGPS(chirp_common.CloneModeRadio):
    ...
    def get_raw_memory(self, number):
        return repr(self._memobj.memory[number - 1].get_raw())
    ...
    def get_memory(self, number):
        ...
        # Tones
        tx_mode, tx_val, tx_pol = _decode_tone(
            _mem.txtone_hi, _mem.txtone_lo)
        rx_mode, rx_val, rx_pol = _decode_tone(
            _mem.rxtone_hi, _mem.rxtone_lo)
        ...
        mem.mode = 'NFM' if bool(_mem.narrow) else 'FM'

        # Skip flag
        scan_byte = int(self._memobj.scan_flags[idx // 8])
        mem.skip = '' if ((scan_byte >> (idx % 8)) & 1) == 0 else 'S'
        ...
        mem.name = _decode_name(_mem.name.get_raw())[:self.LENGTH_NAME]
        ...
        _exa('busylock', 'Busy Channel Lockout',
             RadioSettingValueList(LIST_BCL,
                                   current_index=min(int(_mem.bcl), 2)))
        _exa('txdis', 'TX Disable',
             RadioSettingValueBoolean(bool(_mem.txdis)))
        ...
        _exa('scram', 'Scramble',
             RadioSettingValueBoolean(bool(_mem.scram)))
        _exa('cepin24bit', 'Freq. Measuring 24-bit',
             RadioSettingValueBoolean(bool(_mem.cepin24bit)))
        _exa('cepindcs', 'Freq. Measuring DCS',
             RadioSettingValueBoolean(bool(_mem.cepindcs)))
        _exa('freqinvert', 'Frequency Inversion',
             RadioSettingValueBoolean(bool(_mem.freqinvert)))
        _exa('signaltype', 'Signal Type',
             RadioSettingValueList(LIST_SIGNAL,
                                   current_index=min(int(_mem.signaltype),
                                                     4)))
        _exa('sqtype', 'Squelch Type',
             RadioSettingValueList(LIST_SQTYPE,
                                   current_index=min(int(_mem.sqtype), 3)))
        _exa('fivetoneptt', '5-Tone PTT ID',
             RadioSettingValueList(LIST_PTTID,
                                   current_index=min(int(_mem.fivetoneptt),
                                                     3)))
        _exa('dtmfptt', 'DTMF PTT ID',
             RadioSettingValueList(LIST_PTTID,
                                   current_index=min(int(_mem.dtmfptt), 3)))
        _exa('jumpfreq', 'Jump Frequency',
             RadioSettingValueInteger(0, 3, int(_mem.jumpfreq)))
        _exa('freqstep', 'Frequency Step',
             RadioSettingValueInteger(0, 7, int(_mem.freqstep)))
        _exa('dtmfidx', 'DTMF Index',
             RadioSettingValueInteger(0, 15, int(_mem.dtmfidx)))
        _exa('twotoneidx', '2-Tone Index',
             RadioSettingValueInteger(0, 15, int(_mem.twotoneidx)))
        _exa('fivetoneidx', '5-Tone Index',
             RadioSettingValueInteger(0, 99, int(_mem.fivetoneidx)))
        _exa('mdcidx', 'MDC Index',
             RadioSettingValueInteger(0, 7, int(_mem.mdcidx)))
        _exa('scanlist', 'Scan List',
             RadioSettingValueInteger(0, 9, int(_mem.scanlist)))
        _exa('emerglist', 'Emergency List',
             RadioSettingValueInteger(0, 9, int(_mem.emerglist)))
    ...
    def set_memory(self, mem):
        ...
        if mem.empty:
            self._memobj.memory[idx].set_raw(b'\x00' * _CHN_SIZE)
            _set_chn_used(self._mmap, idx, False)
            return
        ...
        _mem.power = self.POWER_LEVELS.index(mem.power) if mem.power else 0
        _mem.narrow = 1 if mem.mode == 'NFM' else 0
        _mem.offsetdir = _offsetdir_map.get(mem.duplex, 0)
        ...
        if mem.extra:
            for rs in mem.extra:
                ...
                if nm == 'busylock':
                    ...
                    _mem.txdis = 1 if bool(v) else 0
                elif nm == 'talkaround':
                    _mem.talkaround = 1 if bool(v) else 0
                elif nm == 'freqinvert':
                    _mem.freqinvert = 1 if bool(v) else 0
                elif nm == 'compand':
                    _mem.compand = 1 if bool(v) else 0
                elif nm == 'scram':
                    _mem.scram = 1 if bool(v) else 0
                elif nm == 'cepin24bit':
                    _mem.cepin24bit = 1 if bool(v) else 0
                elif nm == 'cepindcs':
                    _mem.cepindcs = 1 if bool(v) else 0
                elif nm == 'signaltype':
                    _mem.signaltype = int(v) & 0x07
                elif nm == 'sqtype':
                    _mem.sqtype = int(v) & 0x0F
                elif nm == 'fivetoneptt':
                    _mem.fivetoneptt = int(v) & 0x03
                elif nm == 'dtmfptt':
                    _mem.dtmfptt = int(v) & 0x03
                elif nm == 'jumpfreq':
                    _mem.jumpfreq = int(v) & 0x03
                elif nm == 'freqstep':
                    _mem.freqstep = int(v) & 0xFF
                elif nm == 'dtmfidx':
                    _mem.dtmfidx = int(v) & 0xFF
                elif nm == 'twotoneidx':
                    _mem.twotoneidx = int(v) & 0xFF
                elif nm == 'fivetoneidx':
                    _mem.fivetoneidx = int(v) & 0xFF
                elif nm == 'mdcidx':
                    _mem.mdcidx = int(v) & 0xFF
                elif nm == 'scanlist':
                    _mem.scanlist = int(v) & 0xFF
                elif nm == 'emerglist':
                    _mem.emerglist = int(v) & 0xFF

        # Name — truncate to LENGTH_NAME chars before encoding so we never
        # store more than the radio can display.
        _mem.name.set_raw(_encode_name(mem.name[:self.LENGTH_NAME], 16))

        _set_chn_used(self._mmap, idx, True)
        ...
        if mem.skip == 'S':
            ...
            scan_byte &= ~(1 << (idx % 8))
        self._memobj.scan_flags[idx // 8] = scan_byte & 0xFF
    ...
    def get_settings(self):
        ...
        _add(basic, 'settings.engineering', 'Engineering Mode',
             RadioSettingValueBoolean(bool((b37 >> 4) & 1)))
        ...
        # ── Key Assignments ──────────────────────────────────────────────────
        key_names = [
            ('settings.skey1', 'Top Button / Red (short)'),
            ('settings.skey2', 'Lower Button / Black (short)'),
            ('settings.lkey1', 'Top Button / Red (long)'),
            ('settings.lkey2', 'Lower Button / Black (long)'),
        ]
        raw_keys = [s[48], s[49], s[50], s[51]]
        ...
        # ── Zones ────────────────────────────────────────────────────────────
        _add(zones, 'zones._note', 'How to set up a zone',
             RadioSettingValueString(
                 0, 80,
                 'Zone name, colon, then channel numbers separated by commas.'
                 '  Example:  Repeaters: 1, 5, 12, 47'
                 '  Max 64 channels.  Leave blank to clear.',
                 False, chirp_common.CHARSET_ASCII + ':,'))
        for zi in range(10):
            base = ADDR_ZONES + zi * 152
            raw_name = _mread(self._mmap, base + 136, 16)
            zname = _decode_name(raw_name)
            chn_num = min(_mb(self._mmap, base), 64)
            chn_ids = []
            ...
            entry_str = ('%s: %s' % (zname, ', '.join(chn_ids))
                         if (zname or chn_ids) else '')
            _add(zones, 'zones.zone%d' % zi, 'Zone %d' % (zi + 1),
                 RadioSettingValueString(0, 270, entry_str, False,
                                         chirp_common.CHARSET_ASCII + ':, '))
            # Preview — all channels resolved to ID:Name
            _preview_parts = []
            for _cid_str in chn_ids:
                try:
                    _cid = int(_cid_str)
                    if _chn_used(self._mmap, _cid - 1):
                        _cname = _decode_name(
                            self._memobj.memory[_cid - 1].name.get_raw()
                        ).strip()
            ...
            _add(zones, 'zones.zone%d_preview' % zi,
                 'Zone %d — channel list (read-only)' % (zi + 1),
                 RadioSettingValueString(0, 500, _preview, False,
                                         chirp_common.CHARSET_ASCII + ',:'))
        ...
        scan_name_raw = _mread(self._mmap, ADDR_SCAN_PARA + 9, 16)
        scan_name = _decode_name(scan_name_raw)
        _add(scan, 'scan.scanname', 'Scan List Name',
             RadioSettingValueString(0, 8, scan_name[:8], False, _CHARSET))
        ...
        # 5-Tone PTT IDs (32 bytes, 4 × 8-byte entries)
        pttid5 = _mread(self._mmap, ADDR_5TONE_PTTID, 32)
        for pi in range(4):
            bp = pi * 8
            plen = min(pttid5[bp], 12)
            pid_str = _decode_5tone_id(bytes(pttid5[bp + 1:bp + 7]), plen)
            _add(fivetone, 'fivetone_pttid.%d_len' % pi,
                 '5-Tone PTT ID %d Length' % (pi + 1),
                 RadioSettingValueInteger(0, 12, plen))
            _add(fivetone, 'fivetone_pttid.%d_id' % pi,
                 '5-Tone PTT ID %d Code' % (pi + 1),
                 RadioSettingValueString(0, 16, pid_str, False,
                                         chirp_common.CHARSET_ASCII))

        # 5-Tone info codes (8 × 16-byte entries)
        for ic in range(8):
            base_ic = ADDR_5TONE_INFO + ic * 16
            icp = _mread(self._mmap, base_ic, 16)
            ic_id = bytes(icp[1:6]).split(b'\x00')[0].decode(
                'ascii', errors='replace')[:5]
            ic_func = min(icp[6], 4)
            ic_rsp = min(icp[7], 2)
            ic_name = bytes(icp[8:14]).split(b'\x00')[0].decode(
                'ascii', errors='replace')[:6]
            _add(fivetone, 'fivetone_info.%d_id' % ic,
                 '5-Tone Info %d Code ID' % (ic + 1),
                 RadioSettingValueString(0, 5, ic_id, False, '0123456789'))
            _add(fivetone, 'fivetone_info.%d_func' % ic,
                 '5-Tone Info %d Function' % (ic + 1),
                 RadioSettingValueList(LIST_5TONE_INFO_FUNC,
                                       current_index=ic_func))
            _add(fivetone, 'fivetone_info.%d_rsp' % ic,
                 '5-Tone Info %d Response' % (ic + 1),
                 RadioSettingValueList(LIST_5TONE_INFO_RSP,
                                       current_index=ic_rsp))
            _add(fivetone, 'fivetone_info.%d_name' % ic,
                 '5-Tone Info %d Name' % (ic + 1),
                 RadioSettingValueString(0, 6, ic_name, False,
                                         chirp_common.CHARSET_ASCII))
        ...
        for mi in range(5):
            ...
            _add(mdc, 'mdc_para.%d_encsync' % mi,
                 'MDC Sys %d Enc Sync' % (mi + 1),
                 RadioSettingValueList(LIST_MDC_SYNC,
                                       current_index=min(encsync, 3)))
            _add(mdc, 'mdc_para.%d_decsync' % mi,
                 'MDC Sys %d Dec Sync' % (mi + 1),
                 RadioSettingValueList(LIST_MDC_SYNC,
                                       current_index=min(decsync, 3)))
            _add(mdc, 'mdc_para.%d_encid' % mi,
                 'MDC Sys %d Enc ID' % (mi + 1),
                 RadioSettingValueInteger(0, 9999, enc_id))
            _add(mdc, 'mdc_para.%d_pretime' % mi,
                 'MDC Sys %d Pre Time (ms)' % (mi + 1),
                 RadioSettingValueInteger(0, 2550, min(pretime_ms, 2550)))
            _add(mdc, 'mdc_para.%d_sqldly' % mi,
                 'MDC Sys %d SQL Delay (ms)' % (mi + 1),
                 RadioSettingValueInteger(0, 2550, min(sqldly_ms, 2550)))
            _add(mdc, 'mdc_para.%d_decrst' % mi,
                 'MDC Sys %d Dec Reset (ms)' % (mi + 1),
                 RadioSettingValueInteger(0, 2550, min(decrst_ms, 2550)))

        # MDC sys list (upper nibble = gen list index,
        #               lower nibble = PTT ID sel)
        sys_raw = _mread(self._mmap, ADDR_MDC_SYS, 8)
        for si_m in range(5):
            packed = sys_raw[si_m]
            gen_idx = min((packed >> 4) & 0x0F, 4)
            pttid_idx = min(packed & 0x0F, 4)
            _add(mdc, 'mdc_sys.%d_genlist' % si_m,
                 'MDC Sys %d Gen List' % (si_m + 1),
                 RadioSettingValueList(_MDC_SYS_LISTS, current_index=gen_idx))
            _add(mdc, 'mdc_sys.%d_pttid' % si_m,
                 'MDC Sys %d PTT ID Sel' % (si_m + 1),
                 RadioSettingValueList(_MDC_SYS_LISTS, current_index=pttid_idx))

        # MDC PTT ID table (5 × 8-byte entries)
        for pi_m in range(5):
            base_pm = ADDR_MDC_PTTID + pi_m * 8
            pmp = _mread(self._mmap, base_pm, 8)
            pttid_val = min(9999, (pmp[0] << 8) | pmp[1])
            pttid_name = _decode_name(bytes(pmp[2:8]))
            _add(mdc, 'mdc_pttid.%d_id' % pi_m,
                 'MDC PTT ID %d' % (pi_m + 1),
                 RadioSettingValueInteger(0, 9999, pttid_val))
            _add(mdc, 'mdc_pttid.%d_name' % pi_m,
                 'MDC PTT ID %d Name' % (pi_m + 1),
                 RadioSettingValueString(0, 6, pttid_name[:6], False, _CHARSET))
        ...
        _add(aprs, 'aprs.txfreqidx', 'APRS TX Freq Index',
             RadioSettingValueInteger(0, 7, min(ap[91], 7)))
        ...
        # GPS Book
        _add(gps_book, 'gps_book._note',
             'How to add a contact',
             RadioSettingValueString(
                 0, 60,
                 'Type a code number, a colon, then the name.'
                 '  Example:  42:HQ Station  '
                 '  Leave blank to clear the slot.',
                 False, chirp_common.CHARSET_ASCII + ':'))
        ...
        for gi in range(80):
            ...
            entry_str = '%d:%s' % (code_id, gname[:7]) if en_flag else ''
            _add(gps_book, 'gps_book.%d' % gi,
                 'GPS Contact %d' % (gi + 1),
                 RadioSettingValueString(0, 12, entry_str, False,
                                         chirp_common.CHARSET_ASCII + ':'))
        ...
        # Model comes from handshake; SW/HW/date are in the mmap at ADDR_DEVICE.
        dev_raw = _mread(self._mmap, ADDR_DEVICE, 32)

        def _devstr(b):
            return bytes(b).split(b'\xff')[0].split(b'\x00')[0].decode(
                'ascii', errors='replace').strip()

        dev_sw = _devstr(dev_raw[0:8])
        dev_hw = _devstr(dev_raw[8:16])
        dev_date = _devstr(dev_raw[16:32])
        _add(devinfo, 'devinfo.model', 'Model',
             RadioSettingValueString(0, 32, self._model, False,
                                     chirp_common.CHARSET_ASCII))
        _add(devinfo, 'devinfo.swver', 'SW Version',
             RadioSettingValueString(0, 16, dev_sw, False,
                                     chirp_common.CHARSET_ASCII))
        _add(devinfo, 'devinfo.hwver', 'HW Version',
             RadioSettingValueString(0, 16, dev_hw, False,
                                     chirp_common.CHARSET_ASCII))
        _add(devinfo, 'devinfo.date', 'Production Date',
             RadioSettingValueString(0, 32, dev_date, False,
                                     chirp_common.CHARSET_ASCII))
    ...
    def _apply_setting(self, rs):  # noqa: C901
        ...
        if name == 'settings.chaworkmode':
            ...
            _wb(35, b35)
                      ...
                      'settings.tianqi', 'settings.engineering',
                      'settings.langsel', 'settings.pownface'):
            b37 = _b(37)
            ...
            elif name == 'settings.engineering':
                b37 = (b37 & 0xEF) | (0x10 if bool(val) else 0)
        ...
        # ── Zones ────────────────────────────────────────────────────────────
        elif name == 'zones._note':
            pass  # read-only instruction label
        elif name.startswith('zones.zone') and '_preview' in name:
            pass  # read-only display field
        elif name.startswith('zones.zone'):
            suffix = name.split('zones.zone', 1)[1]
            if suffix.isdigit():
                zi = int(suffix)
                base = ADDR_ZONES + zi * 152
                raw = str(val).strip()
                if raw and ':' in raw:
                    zname_part, chns_part = raw.split(':', 1)
                    zname = zname_part.strip()[:8]
                    ids = []
                    for token in chns_part.split(','):
                        token = token.strip()
                        if token.isdigit():
                            cid = int(token)
                            if 1 <= cid <= _MAX_CHN:
                                ...
                                LOG.warning(
                                    '5RH: zone %d: channel %d out of '
                                    'range (1-%d), skipped',
                                    zi + 1, cid, _MAX_CHN)
                    if len(ids) > 64:
                        LOG.warning(
                            '5RH: zone %d: %d channels specified, '
                            'truncating to 64', zi + 1, len(ids))
                        ids = ids[:64]
                    _mwrite(self._mmap, base + 136, _encode_name(zname, 16))
                    ...
                    # Blank entry — clear zone
                    _mwb(self._mmap, base, 0)
                    _mwb(self._mmap, base + 1, 0)
                    _mwrite(self._mmap, base + 136, _encode_name('', 16))
                # Auto-update zone count from actual memory content
                cnt = sum(
                    1 for _z in range(10)
                    if _mb(self._mmap, ADDR_ZONES + _z * 152) > 0)
                _mwb(self._mmap, ADDR_ZONE_CNT, cnt)
        ...
        elif name == 'scan.scanname':
            enc = _encode_name(str(val), 16)
            _mwrite(self._mmap, ADDR_SCAN_PARA + 9, enc)
        ...
        elif name.startswith('fivetone_pttid.'):
            parts = name.split('.')
            pi = int(parts[1].split('_')[0])
            field = parts[1].split('_', 1)[1]
            bp = ADDR_5TONE_PTTID + pi * 8
            if field == 'len':
                _mwb(self._mmap, bp, int(val))
            elif field == 'id':
                _mwrite(self._mmap, bp + 1, _encode_5tone_id(str(val), 6))
        elif name.startswith('fivetone_info.'):
            parts = name.split('.')
            ic = int(parts[1].split('_')[0])
            field = parts[1].split('_', 1)[1]
            base_ic = ADDR_5TONE_INFO + ic * 16
            if field == 'id':
                enc = str(val).encode('ascii', errors='replace')[:5]
                enc = enc.ljust(5, b'\x00')
                _mwrite(self._mmap, base_ic + 1, enc)
            elif field == 'func':
                _mwb(self._mmap, base_ic + 6, int(val))
            elif field == 'rsp':
                _mwb(self._mmap, base_ic + 7, int(val))
            elif field == 'name':
                enc = str(val).encode('ascii', errors='replace')[:6]
                enc = enc.ljust(6, b'\x00')
                _mwrite(self._mmap, base_ic + 8, enc)
        ...
        # ── MDC-1200 ─────────────────────────────────────────────────────────
            ...
            if field == 'ctrl':
                ...
                b0 = _mb(self._mmap, base)
                _mwb(self._mmap, base, (b0 & 0xFC) | (int(val) & 0x03))
            elif field == 'decsync':
                b0 = _mb(self._mmap, base)
                _mwb(self._mmap, base, (b0 & 0xF3) | ((int(val) & 0x03) << 2))
            elif field == 'encid':
                v = int(val)
                _mwb(self._mmap, base + 1, (v >> 8) & 0xFF)
                _mwb(self._mmap, base + 2, v & 0xFF)
            elif field == 'pretime':
                _mwb(self._mmap, base + 3, int(val) // 10)
            elif field == 'sqldly':
                _mwb(self._mmap, base + 4, int(val) // 10)
            elif field == 'decrst':
                _mwb(self._mmap, base + 5, int(val) // 10)
        elif name.startswith('mdc_sys.'):
            parts = name.split('.')
            si_m = int(parts[1].split('_')[0])
            field = parts[1].split('_', 1)[1]
            addr = ADDR_MDC_SYS + si_m
            packed = _mb(self._mmap, addr)
            if field == 'genlist':
                packed = (packed & 0x0F) | ((int(val) & 0x0F) << 4)
            elif field == 'pttid':
                packed = (packed & 0xF0) | (int(val) & 0x0F)
            _mwb(self._mmap, addr, packed)
        elif name.startswith('mdc_pttid.'):
            parts = name.split('.')
            pi_m = int(parts[1].split('_')[0])
            field = parts[1].split('_', 1)[1]
            base_pm = ADDR_MDC_PTTID + pi_m * 8
            if field == 'id':
                v = int(val)
                _mwb(self._mmap, base_pm, (v >> 8) & 0xFF)
                _mwb(self._mmap, base_pm + 1, v & 0xFF)
            elif field == 'name':
                enc = _encode_name(str(val), 6)
                _mwrite(self._mmap, base_pm + 2, enc)
        ...
        # ── APRS ─────────────────────────────────────────────────────────────
            ...
            elif field == 'txfreqidx':
                _mwb(self._mmap, base + 91, int(val))
        ...
        # ── GPS Book ─────────────────────────────────────────────────────────
            ...
            if field == '_note':
                ...
                if raw and ':' in raw:
                    parts = raw.split(':', 1)
                    try:
                        code = max(0, min(255, int(parts[0].strip())))
                    except ValueError:
                        code = 0
                    gname = parts[1].strip()[:7]
                    _mwb(self._mmap, base, code)
                    _mwb(self._mmap, base + 1, 0)
                    _mwrite(self._mmap, base + 2, _encode_name(gname, 14))
